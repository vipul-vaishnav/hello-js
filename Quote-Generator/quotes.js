...
const num_of_quotes = localQuotes.length;
const quoteTitle = document.querySelector('.quote-title');
const quoteAuthor = document.querySelector('.quote-author');
const newQuote = document.querySelector('.new-cta');

function getData() {
  const randomNum = Math.floor(Math.random() * num_of_quotes);
  const quoteText = localQuotes[randomNum].text;
  quoteTitle.innerHTML = `
  <span class="title-icon">
    <i class="ri-double-quotes-l"></i>
  </span>
  <span class="quote-title-text">
    ${quoteText}
  </span>
  `;

  const quoteAuthorName = localQuotes[randomNum].author == null ? 'unknown' : localQuotes[randomNum].author;
  quoteAuthor.innerHTML = `
  ~
  <span class="quote-author-text">
      ${quoteAuthorName}
  </span>
  `;

  quoteTitle.classList.remove('animated_bg');
  quoteAuthor.classList.remove('animated_bg');
}

setTimeout(getData, 2500);

newQuote.addEventListener('click', () => {
  quoteTitle.classList.add('animated_bg');
  quoteAuthor.classList.add('animated_bg');
  quoteTitle.innerHTML = ` `;
  quoteAuthor.innerHTML = ` `;
  setTimeout(getData, 2500);
});
